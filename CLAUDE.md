...
### Backup / restore
`BackupService` + `BackupApiController` (`api/backup/*`, Settings → Backup) produce and consume one zip holding `manifest.json`, a `pg_dump --format=custom` of the whole database, and the `Uploads/` tree. Notes if you touch it:
- **Instance-wide, not tenant-scoped** — the dump covers every user's rows (including password hashes) and a restore replaces all of them, so the controller is `[Authorize(Roles = Admin)]` and does *not* derive from `TenantControllerBase`.
...
- `NHibernateHelper.Describe()` is what exposes the resolved provider/connection string outside NHibernate — it re-reads the layered config rather than caching what `SessionFactory` built, so it doesn't depend on init order.
